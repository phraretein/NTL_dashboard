...
def get_goalcompletion(date):
	...
	for index, row in goal.iterrows():
		count.append(get_keywordcount(date,row.Keywords,by='user'))
	goal['Count'] = count
...
def get_converinterval(date):
	chatlog_conv = get_conver(date)
	conv_interval = []
	chatlog_conv = chatlog_conv.sort_values(by='conv',ascending=True)
...
def get_keywordcount(date, keywordlist, by='message'):
	...
	if by == 'message':
		...
		keywordcount = len(dat.loc[dat.key==1].userId.unique())
	else:
		keywordcount = 0
